...
- 检查全绿（build + test 31 文件 444 用例 + conservation +
  lint/format 零输出）
- 全量守恒零漂移；changelog 风格门 PASS
- Playwright 套件 preview 全绿 + 线上复跑全绿
- fmt 新单测 4 断言全过，既有 26 用例零回归

---

## v0.88.3 — 修复: 行动队列按压回弹与分段切换器注记收口

**变更性质：修复（交互微交互与文档注记收口，无数值/存档变更）**
**开发时间：2026-09-13**

### 概述

行动队列可执行项按压回弹对齐《动效规范》定标：此前基础过渡 0.2s
直通按压态，缺 0.1s 快速段，与文明概况行及文档参数明细不一致；
组件规范分段切换器表两处过期差异注记回写为已落地口径。

### 变更明细

- ActionQueuePanel `.action-item.actionable:active` 补
  `transition: transform 0.1s` 覆盖，按压 0.1s 快速段、释放回 0.2s，
  与 ov-item 及文档参数明细对齐
- 组件规范分段切换器表：padding 行「待代码收敛」改三处一致（v0.88
  统一定标）；激活段 aria 行「aria-pressed 待补」改三处齐备（v0.88
  落地）
...
## v0.88.1 — 修复: 弱化文本色提值与视觉稿归档

**变更性质：修复（无障碍对比度提值 + 文档瘦身，无数值变更）**
**开发时间：2026-09-12**

### 概述

弱化文本色提亮达到无障碍标准：三底色实测全部达标，四十余处
辅助文本可读性提升；组件规范六处视觉稿图示归档，正文保持
现行参数。

### 变更明细

- 弱化文本色由 #6B7589 提为 #7A8699：对底色实测 5.46、面板底
  4.98 双双达到无障碍标准（原值 4.35 不达标，高亮底也从
  3.42 提至 4.30）；令牌注释与四份规范文档同步登记实测值
- 可访问性章处置节改写：从「限用场景加已知局限」更新为
  「已提值达标」，附三底色对比表
- 组件规范六处视觉稿图示归档至历史存档（英雄区两端、行动
  队列、桌面双列布局两段、文明概况卡结构），正文留指针，
  参数表与验收不动；组件规范净减 137 行

### 验证

- 程序实算对比度选色（候选四色对三种底色全测），不采信约数
- corepack pnpm check 全绿：构建、31 文件 443 用例、计数守恒、
  lint 与格式零输出
- 四页观感截图走查（首页/建造/部队/转生）确认层次自然
- 双端九路由回归全过（线上侧随部署复跑）
...
**变更性质：修复（交互规范定标小项五处 + 训练失败提示，无数值变更）**
**开发时间：2026-09-12**

### 概述

规范核对遗留的代码小项收口：分段切换器统一、锁定卡名迁移收口、
焦点偏移归位、四视图卡片按压回弹补齐；训练失败路径补明示提示。

### 变更明细

- 分段切换器三处内边距统一为 4px 12px 定标值，激活段补
  aria-pressed 无障碍状态
- 锁定卡名称迁移收口：星图与建造视图锁定卡名称及描述迁锁定
  灰蓝色（部队视图随上一版本已迁）
- 倒计时明细面板的焦点偏移 1px 归位全局 2px
- 建造/科技/星图/文明概况四视图卡片补按压回弹（上浮回落 +
  微缩 0.98，0.1 秒），与动效规范定标一致
- 训练失败路径补提示：满槽与资源不足不再静默失败（覆盖连点
  竞态等禁用更新前的边缘场景）
...
- corepack pnpm check 全绿：构建、31 文件 443 用例、计数守恒、
  lint 与格式零输出
- 锁定色守护专项与批量升级专项回归全过
- 冒烟与双端九路由回归全过（线上侧随部署复跑）

---

## v0.87.1 — 修复: 状态色与按钮反馈六处体验修正

**变更性质：修复（六处玩家可感知状态/反馈缺陷，无数值变更）**
**开发时间：2026-09-12**

### 概述

设计规范核对批的代码侧六处全部修复：成本标签、锁定卡、强化
弹窗、行动队列与禁用按钮的反馈口径统一到规范定标。

### 变更明细

- 成本标签资源不足由红色改琥珀：红色保留给不可逆操作，资源
  不足可逆属警告级
- 部队页锁定卡不再降低透明，卡名迁锁定灰蓝色（与其他视图
  锁定卡口径一致）
- 强化弹窗默认 ×1 时按钮恢复原文案「强化」，与建造/转生批量
  口径一致
- 强化按钮补禁用逻辑：一级都买不起时禁用，不再点击后提示
...
- corepack pnpm check 全绿：构建、31 文件 443 用例、计数守恒、
  lint 与格式零输出
- 单测改写一处：能量不足用例从「点击提示」改为「按钮禁用」
  断言，与禁用口径互证
- 批量升级专项断言同步（×1 文案期望改原文案）
- 冒烟与双端九路由回归全过

---

## v0.87 — 修复: 导入二次确认与弹窗收敛

**变更性质：修复（四处玩家可感知体验修正，无数值与存档结构变更）**
**开发时间：2026-09-12**

### 概述

随文档体系优化落地的四处玩家可感知体验修正：导入存档补二次确认
防误触覆盖；离线报告弹窗收敛到通用弹窗组件；满槽训练倒计时接入
读屏播报；导出存档码补无障碍标签；时间加速仿真注释定性更新。
两册新规范同步成册，docs 达十一册目标态。

### 变更明细

- 导入二次确认：转生页点导入先弹确认框，明示完全替换与不可恢复
  语义，确认后才执行解析与全量替换；取消或点遮罩不触碰现有存档
- 离线报告收敛：内联弹窗迁移到通用弹窗组件，入场动画 0.3s 统一
  为 0.25s，硬编码阴影接 elevation 令牌，焦点陷阱交由组件提供，
  遮罩遗留空类清除
- 无障碍两项：满槽训练按钮的倒计时文字包读屏播报区；导出存档码
  常显文本域补标签
- 仿真注释：时间加速仿真定性更新为已收编常规测试基线，用例名
  去掉内部阶段编号
- 新规范两册：存档与数据安全规范收编存储双通道、版本机制、读取
  四态、校验自愈、写入守卫、导入导出、错误屏、存档管理区与安全
  边界；弹窗与确认流规范定标确认流三级分级、弹窗结构标准与七处
  确认流
...
- corepack pnpm check 全绿：构建通过、31 文件 443 用例、计数守恒、
  lint 与格式零输出
- 专项回归：存档安全、冒烟、双端九路由、冒烟检查 17 项全过
- 新增两项专项各 7 断言：离线报告正路（弹出、结构、关闭）与导入
  确认流（弹窗、取消不执行、确认才替换）全过
- 中英文 readme 文档索引达十一册

---

## v0.86.2 — 修复: 全玩法 QA 三项体验修正

**变更性质：修复（QA 走查产出的三项体验修正；数值与安全行为零变化）**
**开发时间：2026-09-12**

### 概述

全玩法时间加速 QA（仿真 6 场景 + 真实浏览器全流程走查 + 安全矩阵）
未发现功能与安全级缺陷，产出三项体验改进：导出存档码改为常显、
满槽训练按钮显示剩余时间、临时仿真测试收编入基线。

### 变更明细

- 导出存档（PrestigeView）：存档码始终展示在只读文本域，不再仅在
  自动复制失败时回退显示；http 等非安全上下文下剪贴板可能静默失
  败，常显保证玩家永远有手动复制兜底
- 训练按钮（ArmyView）：训练槽满时按钮文案由死「训练」改为「训练
  中…剩 Xs」（最早完成队列的实时剩余时间），消除新手「按钮坏了」
  的误解；未满槽时文案与原样一致
- 新增时间加速仿真测试 6 场景（产出守恒/训练链/探索链/转生重置
  范围/12h×2 极端加速/自动化协议）收编为常规基线；测试基线 31 文
  件 443 用例，README 同步
- 全量安全矩阵确认为通过态：存档篡改 6 向全拒、XSS 不执行、连点
  零重入、恶意导入（含原型污染）全拒

### 验证

- 一键检查全绿：测试 31 文件 443 用例 + 守恒 + lint 与 format
  零输出
- Playwright 走查与回归套件复跑通过；满槽训练按钮实时倒计时目视
  确认；导出后文本域常显存档码确认

---

## v0.86.1 — 修复: 转生树节点成本显示接入统一格式化

**变更性质：修复（两处显示层修正；数值与交互零变化）**
**开发时间：2026-09-12**

### 概述

转生页天赋树节点的成本显示为裸数字，未走全局 fmt 格式化管线，
违反「千以下直接显示、千以上 K/M/B/T 进位」的数值显示规范。无限
节点成本按 ceil(基础 × 1.5^等级) 指数增长，高等级下出现超长裸数
（如 Lv.40 约 11 万亿直接铺开），与同页负熵余额的格式化显示形成
反差。本次为买断与无限两处节点成本套上 fmt，微小显示层修正。

### 变更明细

- PrestigeView 买断节点成本 {{ node.cost }} 改 {{ fmt(node.cost) }}，
  无限节点成本 {{ nextCost(node) }} 改 {{ fmt(nextCost(node)) }}
- 现值全部低于 1000，fmt 输出与裸显逐字节相同，既有显示零变化；
  仅高等级无限节点未来超出千位时自动进位（如 33.25K 负熵）

### 验证

- 一键检查全绿：测试 30 文件 437 用例 + 守恒 + lint 与 format
  零输出；v056 专项成本断言（5/8/12 负熵）preview 复跑通过
- 显示规范全量排查同步进行：五资源数量显示均确认走 fmt 管线，
  本次两处为全部违例

---

## v0.86 — 功能: 批量升级：等级型购买入口 ×1/×10/×100 段位切换

**变更性质：功能（三处等级型购买入口新增批量档位；存档零变化）**
**开发时间：2026-09-11**

### 概述

建筑升级、转生树无限天赋、遗物强化三处此前均只能点一次升一级，
后期等级高时连点繁琐。本次为三处入口新增 ×1/×10/×100 段位切换
器，点击按当前段位批量购买，采用买满语义：预算中途耗尽或达等级
上限自然停止，一级都买不起时按钮禁用。批量内部逐级复用既有原子
操作，成就、周挑战等记账粒度与手动连点完全等价，存档结构零变化。
...
- 建造页：页头新增段位切换器（扇区标签上方，全部建筑卡共用），
  段位大于 1 时主按钮文案带后缀（「升级 ×10」）
- 转生树：「无限天赋」区标题行内新增切换器；买断节点均为一级封
  顶，不显示切换器；默认段位 ×1 与既有行为一致
- 遗物强化：弹窗按钮行内新增切换器与「强化 ×N」主按钮；关闭按钮
  补 data-testid（enhance-close）；默认段位 ×1
- docs/交互状态规范新增 1.12 批量购买口径（买满语义、记账粒度、
  切换器位置、文案规则、自动化协议不受影响）
- 自动化建造协议保持每建筑每 tick 至多 1 级，与手动批量独立

### 验证

- 一键检查全绿：测试 30 文件 437 用例（新增 12 个批量语义用例：
  买满、中途截断、零购买、上限截断、与连点记账等价）+ 守恒 +
  lint 与 format 零输出；README 测试基线 425 → 437 同步
- 专项脚本 starcore-v086-bulk-upgrade.mjs 24 断言全过：三处默认
  ×1 原行为、×10 连升与买满截断、×100 一键买满（转生 Lv.21、
  遗物拉满 20）、移动 375 无溢出
- 回归十脚本全过：batchA、v045 双端、v046/048/049、v056（转生
  默认链兼容）、v058（自动化不受影响）、v061（强化默认链兼容）、
  v082；三处切换器截图目视验证通过
