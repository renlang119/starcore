...
**变更性质：修复（远征模板校验、深度封顶、奖励即时发放与文案统一）**
**开发时间：2026-09-14**
...
- 测试：新增敌方空编成防御、四模板轮换位编成非空、远征深度封顶
  三条用例；奖励防重复用例改写为即时到账口径
- README 双语测试基线随用例新增同步 458 至 460（31 个测试文件）

### 验证

- 全量测试 31 文件 460 用例通过，构建类型检查、lint 与格式检查零
  输出，全量计数守恒通过
...
防误触覆盖；离线报告弹窗收敛到通用弹窗组件；设计规范核对批的
