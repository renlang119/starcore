...
- **部队**：4 种兵种双三角克制，训练并行槽 1→3（科技扩展）；3 支编队各自可切换特性
  （强攻/坚壁/破敌/后勤），战斗与驻扎分工配置
- **据点 PVE**：4 类 36 据点，胜利后可驻扎挂机产出；另有深度递增的「无尽远征」，
  每攻克 10 层可领取一档里程碑奖励（跨转生累计，漏领可补），每周一另刷新
  一只锚定玩家远征前沿的每周强敌（v1.24）
...
- **单元/组件测试**：Vitest + @vue/test-utils + jsdom，42 个测试文件 617 个用例
