...
describe('research — 数值规范守恒（docs/游戏数值设定规范.md §四）', () => {
  ...
  it('全研究后各资源科技乘数（规范 §四 口径）', () => {
    ...
    expect(store.getMult('production_mult', 'data').toNumber()).toBeCloseTo(7.49112, 10)
    expect(store.getMult('production_mult', 'dark').toNumber()).toBeCloseTo(5.99781, 10)
    expect(store.getMult('combat_mult', 'attack').toNumber()).toBeCloseTo(3.80859375, 10)
    expect(store.getMult('combat_mult', 'defense').toNumber()).toBeCloseTo(3.80859375, 10)
    expect(store.getMult('explore_mult').toNumber()).toBeCloseTo(11.6957295, 10)
