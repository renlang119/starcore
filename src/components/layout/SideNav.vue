...
function toggleCollapsed() {
  ...
  try { localStorage.setItem(COLLAPSE_KEY, String(collapsed.value)) } catch { /* */ }
}
</script>
...
<template>
  <nav class="side-nav" :class="{ collapsed }">
    ...
    <button
      ...
      @click="nav(n.path)"
    >
      <svg class="icon" style="width: var(--icon-md); height: var(--icon-md)" aria-hidden="true"><use :href="'#' + n.icon" /></svg>
...
.side-nav.collapsed { width: 48px; padding: var(--space-4) var(--space-2); }
...
.brand-name {
  font-size: var(--text-base); color: var(--color-core);
  white-space: nowrap; overflow: hidden;
  transition: opacity .15s var(--ease-out);
}
.side-nav.collapsed .brand-name { opacity: 0; }
...
.nav-item:hover { background: var(--color-hover); color: var(--color-t-primary); }
.nav-item.active {
...
.side-nav.collapsed .nav-label { opacity: 0; }
.side-nav.collapsed .nav-item { justify-content: center; }
...
.side-nav.collapsed .side-footer { opacity: 0; }
.stat { display: flex; justify-content: space-between; font-size: var(--text-xs); }
.stat .label { color: var(--color-t-tertiary); }
.stat .val { color: var(--color-t-primary); }
...
.collapse-toggle:hover { background: var(--color-hover); color: var(--color-t-primary); }
.collapse-toggle svg { transition: transform .2s var(--ease-out); }
</style>
