...
> 本文件为历史存档，不再更新；v0.51 起的条目见 [changelog.md](changelog.md)。
> 条目已按 v0.75 条目风格统一整理，新 → 旧倒序。
>
> **完整性注记（v0.83 校勘）**：
> 1. **条目缺佚**：v0.21-23 与 v0.25-28 共 7 条无存档记录（早期版本未留档），
>    按「无记录不编造」原则不作补写，条目序号在此留空。
...
## v0.24 — 修复: 代码评审 P2/P3 修复与 TypeScript 严格度提升
...
训练数量 `trainCount` 默认值为 1，用户进入兵营页面时每个兵种默认显示训练 1
...
而是错误的数值（多 1 或少 1），导致用户设定训练数量时出现偏差。
...
用户反馈昨晚开始训练的部队进度条才走了约三分之一。根因是 `game.ts` 的
...
「奇点重启」两个入口；原 `AppShell.vue` 的 `extra-nav` 浮动栏仅在用户已处
