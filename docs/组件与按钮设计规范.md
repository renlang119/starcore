...
| 参数 | 定标值 | 现存差异 |
|---|---|---|
| padding | `var(--space-1) var(--space-3)`（4px 12px） | PrestigeView 2px 8px、EnhanceModal 0px 8px，待代码收敛 |
| 字号/字体 | `--text-xs` + mono | 三处一致 |
| 激活色 | 跟上下文主色（core 青 / amber 琥珀） | 三处语义一致 |
| 激活段 aria | 容器 `role="group"` + `aria-label` 已有；激活段 `aria-pressed` 待补（代码待办） |
...
> 《动效规范》§1.2，本章不重复定义；Build/Tech/Map/Overview 四视图卡片现仅
> hover 无 active，登记为「未落地」，新增卡片时按《动效规范》补齐。
> Modal 弹窗的确认流分级与结构标准见第 11 册《弹窗与确认流规范》（待建），
...
| 语义定标 | not-enough 应为 `--color-amber`（资源不足可逆，警告级）；现行 alert 红为待代码修复（A-3） |
| 使用点 | Build/Tech/Army 全站成本显示（4 处复用） |
...
| 项 | 内容 |
|---|---|
| 文件 | `src/components/ui/ModalOverlay.vue`；内联双轨实现 OfflineReport 待收敛（C-9，阶段 5 处理） |
...
| 现行差异 | OfflineReport 内联弹窗 modalIn 0.3s + 硬编码阴影，收敛后以《动效规范》时长总表为准 |
