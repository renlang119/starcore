...
容器 `role="group"` + 分段按钮。v1.02 起与容器 `.bulk-toggle` 一并统一为全局类
...
> **实现注记**：aria-pressed 与 padding 转为代码待办，不改写现行行为。
...
| 结构 | 单条浮层，`role="status"` + `aria-live="polite"`，v0.73 自三处重复实现合并 |
...
| 项 | 内容 |
|---|---|
| 文件 | `src/components/ui/ModalOverlay.vue`；OfflineReport 已并入 ModalOverlay（v0.87，全站单轨） |
