...
`.seg-btn` 是段位切换器（×1/×10/×100）专用形态，为视图 scoped 类（三处各自携带、
未收敛为全局类），语义上属于按钮系统的第五种形态：容器 `role="group"` + 分段按钮。
...
.seg-btn.active {
  /* 激活色跟随所在上下文主色：建造/强化页 core 青，转生页 amber 琥珀 */
  color: var(--color-core);            /* 或 var(--color-amber) */
  background: rgba(0, 229, 255, 0.1);  /* 对应主色 10% 底 */
}
```
...
| PrestigeView（无限区标题行） | 内联 | ×1/×10/×100 |
| EnhanceModal（弹窗按钮行） | 内联 | ×1/×10/×100 |
...
| 行为 | `action` + `to` 同传时路由跳转；仅 `action` 时 emit('action') 交视图处理 |
| 死按钮约束 | 不传 `to` 又无 @action 监听即死按钮，使用点必须验证有真实去向（v0.48 教训） |
