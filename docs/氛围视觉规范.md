...
> **边界声明**：本文档只管色彩调色板与视觉参数，不定义状态色语义；amber / alert / locked 等状态色语义唯一出处为《交互状态规范》
...
在现有 `.star-field` 星点层（DOM）基础上，新增 `body::after` 星云层 + 屏幕四周 inset 暗角，构建三层纵深：

```
z-index 层级（从底到顶）：
  .star-field   →  星点层（DOM，AppShell 生成，z:0）
...
|---|---|---|:---:|:---:|
| 星点层（已有） | ~~`body::before`~~ → `.star-field`（DOM） | 星点 DOM 节点（AppShell v-for 生成 + twinkle 动画） | 0 | 0.7 |
| 星云+暗角（新增） | `body::after` | 3 个大椭圆 `radial-gradient` + inset `box-shadow` | 0 | 1（不设整体 opacity，由各 gradient 内部控制） |

> **实现注记（v0.86.2 回写）**：星点已 DOM 化为 `.star-field`（AppShell v-for 生成 + twinkle
> 动画），原 `body::before` 背景方式已移除；opacity 0.7 不变。

> `.star-field` 与 `body::after` 同为 `z-index: 0`，渲染顺序 `body::after` 在星点层之上，星云层自然覆盖在星点层上方，但两者不透明度都极低，不会互相遮蔽。
...
> **实现注记（v0.63 回写，v0.86.2 修正）**：落地为三档 phase（idle/progress/active），
> 环色恒定、不随 phase 变化：r1=--color-quantum（绿）/ r2=--color-plasma（紫）/
> r3=--color-amber（琥珀）（以 HeroCore.vue 实测为准），与早期稿参数不同。
...
> **落地差异（v0.83 回写）**：实现为 `.hero-flow` 独立 div（HeroCore.vue）而非本节
> 原稿的 `.hero::after` 伪元素（`.core-visual::after` 已被渗透光晕占用的顾虑不变，
> 实现改用实元素规避）；形态为横向椭圆光晕（120×60px，向下渐隐 + flowPulse 呼吸
> 动画），非本节的 2px 竖直细线。`.hero::after` 代码块保留为历史设计稿。
...
> **平移注记（2026-09-12）**：光柱呼吸动画（`flowPulse`）参数已平移至《动效规范》
> §4.3（实测周期 2.5s，与核心脉动 3s 各自独立）。动画为可选项：若评估性能或
> 视觉过度，可不加动画，光柱静态渐隐已有足够引导效果；`prefers-reduced-motion`
> 降级策略见《动效规范》第六章。
...
| 动画（可选） | `flowPulse` 参数见《动效规范》§4.3（实测 2.5s） |
...
| 名称 | 用途 | 对应任务 |
|---|---|---|
| ~~`flowPulse`~~ | P2-9 光柱呼吸动画（可选），已平移至《动效规范》§4.3 | P2-9 |
