...
> 原处保留指针注记。
> **同步状态**：基线 v0.63（2026-07-16）｜最后核对 v1.06（2026-09-18）
...
> **偏差登记（v0.86 核对；已销项）**：原「QuickActions 4 处与 useActionQueue 8 处硬编码 #00E5FF/#A78BFA/#2EE6A0/#F43F5E」已消除：QuickActions 随组件层主题色 token 化（v0.96）、useActionQueue 随数据层色值令牌化（v1.08）先后改为引用系统主色 Token，与「图标颜色 = `var(--c)` 系统主色」定标一致。
...
| 引导按钮 | `.btn-secondary.sm` | P1-5 按钮系统；`action` + `to` 同传才渲染，点击跳转路由 |

> **硬规则**：空态引导按钮必须配真实去向——使用点必须传 `to`；组件层仅传
> `action` 不渲染按钮，从结构上杜绝死按钮（v0.48 实装教训）。
...
| TechView | 当前筛选分支全部完成（全部锁定仍展示锁定行，不触发空态） | `i-nav-tech` | 全量筛选：所有已知科技已研究完成；分支筛选：「分支名」分支的科技已全部研究完成（v0.97 口径分离） | 探索新星域可能发现未知科技 | [前往探索] → `/map` |
| MapView | 可探索节点（全部完成/锁定） | `i-nav-explore` | 全宇宙已探索完毕 | 先驱者的航路与信号就此走完，星核文明接过了守门者的位置 | [前往科技] → `/tech` |
| BattleView | 据点不存在（越界/未知据点 id，整页兜底） | `i-nav-explore` | 据点不存在 | 返回星图重新选择目标 | [返回星图] → `/map` |
...
气泡自身仅声明 `animation: floatUp 0.3s var(--ease-out)`；开启减弱动画时由
`src/styles/base.css` 的全局规则统一把动画压到 0.01ms 单次，等效瞬时切换。
本条不再定义独立的降级关键帧，全局规则见《动效规范》第六章。
...
| 验收项 | 标准 |
|--------|------|
| 空状态文案 | 8 种空状态场景均有文案（HomeView 行动队列为轻量空态，无图标/按钮） |
...
| 多气泡顺序 | 同页多气泡按顺序显示 |
| reduced-motion | 由全局降级规则统一切换（《动效规范》第六章） |
...
| `i-nav-home` | 侧栏导航主界面项（HomeView 无空态图标用法） |
